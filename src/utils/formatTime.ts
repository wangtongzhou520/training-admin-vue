 ...
 * 获取当前的日期+时间
 */
export function getNowDateTime(): Dayjs {
  return dayjs()
}
 ...
 * @returns 返回拼接后的时间字符串
 */
export function formatPast(param: string | number | Date, format?: string): string {
  // 传入格式处理、存储转换值
  let t: number
  let s: number
  // 获取js 时间戳
  ...
  if (time < 10000) {
    ...
    // 超过3天
    const date =
      typeof param === 'string' || typeof param === 'object' ? new Date(param) : new Date(param)
 ...
 * @returns 返回拼接后的时间字符串
 */
export function formatAxis(param: Date | string | number): string {
 ...
 * @param cellValue 字段值
 */
export const dateFormatter2 = (row: any, column: any, cellValue: any): string | undefined => {
  if (!cellValue) {
    return undefined
  }
...
export function betweenDay(param1: Date | string, param2: Date | string): number {
  const date1 = convertDate(param1)
  const date2 = convertDate(param2)
  // 计算差值
 ...
 * @param param2 添加的时间
 */
export function addTime(param1: Date | string, param2: number): Date {
 ...
 * @param days 天数
 */
export function getDayRange(date: Date | string, days: number): string[] {
 ...
 * @param endDate 截止日期
 */
export function getDateRange(beginDate: Date | string | Dayjs, endDate: Date | string | Dayjs): string[] {
  return [
